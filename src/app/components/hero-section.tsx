    ...
    <>
        ...
        <div className="flex items-center justify-between bg-neutral-900 px-4 py-3 shadow-lg md:px-8">
          ...
          <div className="flex items-center gap-2">
            <a href={LINE_URL} target="_blank" rel="noopener noreferrer"
              className="rounded-full bg-[#06C755] px-3 py-2 text-xs font-bold text-white hover:opacity-90 md:px-4">
              無料で試す
            </a>
            <button onClick={() => document.getElementById(pricingId)?.scrollIntoView({ behavior: "smooth" })}
              className="rounded-full bg-white px-3 py-2 text-xs font-bold text-neutral-900 hover:bg-neutral-100 md:px-4">
              料金を見る
            </button>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden bg-[#FAFAF8] px-5 pb-16 pt-14 md:px-6 md:pb-32 md:pt-24">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          {/* 권위 배지 */}
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-neutral-200 bg-white px-4 py-2 text-xs font-semibold text-neutral-600 shadow-sm md:text-sm">
            🇰🇷 韓国の入試家庭教師兼小説家が、構成から執筆まで直接制作
          </div>

          {/* 메인 카피 */}
          <h1 className="mb-5 text-[30px] font-extrabold leading-[1.4] tracking-tight text-neutral-900 md:text-[46px] md:leading-[1.25]">
            韓国語、何度始めても<br />
          ...
          {/* 서브 카피 */}
          <p className="mx-auto mb-3 max-w-xl text-[15px] leading-[1.85] text-neutral-600 md:text-lg">
          ...
          <p className="mx-auto mb-10 max-w-xl text-sm leading-[1.85] text-neutral-500 md:text-base">
            小説のように読める教材 × 毎週届く個別単語帳 × 個別QnAコーチング。<br />
            「続かない」を、構造から変えます。
          </p>

          {/* 타깃 레벨 */}
          <p className="mx-auto mb-8 max-w-xl rounded-xl border border-neutral-200 bg-white px-5 py-3.5 text-[13px] font-medium leading-relaxed text-neutral-500 md:text-sm">
          ...
          <div className="mb-10 grid grid-cols-1 gap-3 text-left md:grid-cols-3">
            {[
              ...
              <div key={p.title} className={`flex items-start gap-3 rounded-2xl border p-5 ${p.icon === "🎯" ? "border-orange-200 bg-orange-50" : "border-neutral-200 bg-white"}`}>
                ...
                <div>
                  <p className={`text-[15px] font-bold ${p.icon === "🎯" ? "text-orange-800" : "text-neutral-900"}`}>{p.title}</p>
                  <p className={`mt-1.5 text-sm leading-relaxed ${p.icon === "🎯" ? "text-orange-700" : "text-neutral-500"}`}>{p.desc}</p>
                </div>
              </div>
            ))}
          </div>
